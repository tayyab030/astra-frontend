...
export default function TasksPage() {
  ...
  const projects = [
    { id: "astra-dashboard", name: "ASTRA Dashboard", icon: Target, color: "purple", taskCount: 12 },
    { id: "quarterly-review", name: "Quarterly Review", icon: BarChart3, color: "green", taskCount: 7 },
    { id: "health-tracking", name: "Health Tracking", icon: Zap, color: "red", taskCount: 4 },
  ]
  ...
  const handleDrop = (sectionId: string) => {
    if (draggedTask) {
      ...
      setTaskList((prevTasks) =>
        prevTasks.map((task) =>
          task.id === draggedTask ? { ...task, status: statusMap[sectionId] || task.status } : task,
        ),
      )
      ...
      setSectionList((prevSections) =>
        prevSections.map((section) => {
          if (section.id === sectionId) {
            return {
              ...section,
              tasks: section.tasks.includes(draggedTask!) ? section.tasks : [...section.tasks, draggedTask!],
            }
          } else {
            return {
              ...section,
  ...
  const filteredTasks = taskList.filter((task) => {
    if (selectedProject !== "my-tasks" && task.project !== selectedProject) return false
    ...
    if (calendarView === "month") {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        days.push(
          <div
            key={day}
            className={`h-24 border border-slate-700/30 p-1 ${isToday ? "bg-cyan-500/10 border-cyan-500/30" : "hover:bg-slate-800/30"
              }`}
          >
            <div className={`text-sm font-mono mb-1 ${isToday ? "text-cyan-300" : "text-slate-300"}`}>{day}</div>
            <div className="space-y-1">
              {tasksForDay.slice(0, 2).map((task) => (
                <div
                  key={task.id}
                  className={`text-xs p-1 rounded truncate ${getPriorityColor(task.priority)} bg-opacity-20 text-white font-mono`}
                  title={task.title}
                >
        ...
        <div className="grid grid-cols-7 gap-0">
          {weekDays.map((day, index) => {
            ...
            return (
              <div
                key={index}
                className={`border border-slate-700/30 p-2 min-h-[300px] ${isToday ? "bg-cyan-500/10 border-cyan-500/30" : "hover:bg-slate-800/30"
                  }`}
              >
                <div className={`text-sm font-mono mb-2 ${isToday ? "text-cyan-300" : "text-slate-300"}`}>
                  {day.toLocaleDateString("en-US", { weekday: "short", day: "numeric" })}
                </div>
                <div className="space-y-2">
                  {tasksForDay.map((task) => (
                    <Card key={task.id} className="bg-slate-900/50 border-slate-600/30 backdrop-blur-sm">
                      <CardContent className="p-2">
                        <div className="flex items-center space-x-2">
                          <div className={`w-2 h-2 rounded-full ${getPriorityColor(task.priority)}`} />
                          <span className="text-xs font-mono text-slate-200 truncate" title={task.title}>
                            {task.title.length > 25 ? `${task.title.substring(0, 25)}...` : task.title}
                          </span>
                        </div>
                        <div className="text-xs text-slate-400 mt-1 font-mono">{task.assignee}</div>
                      </CardContent>
                    </Card>
                  ))}
                </div>
      ...
      for (let month = 0; month < 12; month++) {
        ...
        months.push(
          <div
            key={month}
            className={`border border-slate-700/30 p-3 rounded-lg ${isCurrentMonth ? "bg-cyan-500/10 border-cyan-500/30" : "bg-slate-900/30 hover:bg-slate-800/50"
          ...
          >
            <div className={`text-sm font-mono font-bold mb-2 ${isCurrentMonth ? "text-cyan-300" : "text-slate-300"}`}>
              {monthName}
            </div>
            ...
            <div className="grid grid-cols-7 gap-px text-xs">
              {["S", "M", "T", "W", "T", "F", "S"].map((day, i) => (
                <div key={i} className="text-slate-500 text-center font-mono h-4 flex items-center justify-center">
                  {day}
                </div>
              ))}
            ...
            {tasksInMonth > 0 && (
              <div className="mt-2 flex items-center justify-center">
                <div className="bg-cyan-500/20 border border-cyan-500/30 rounded-full px-2 py-1">
                  <span className="text-xs font-mono text-cyan-300">{tasksInMonth} tasks</span>
                </div>
              </div>
            )}
          </div>,
                ...
                >
                  <div className="flex items-center space-x-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2">
                        <h3 className="font-medium font-mono text-slate-200 truncate">{task.title}</h3>
                        <div className={`w-2 h-2 rounded-full ${getPriorityColor(task.priority)}`} />
                      </div>
                      {task.description && (
                        <p className="text-sm text-slate-400 font-mono truncate mt-1">{task.description}</p>
                      )}
                    </div>
                      ...
                      {section.tasks.map((taskId) => {
                        ...
                        return (
                          <Card
                            ...
                            onDragStart={() => handleDragStart(task.id)}
                            onDragEnd={handleDragEnd}
                            className={`bg-slate-900/50 hover:bg-slate-800/50 transition-all duration-200 cursor-grab active:cursor-grabbing border-slate-600/30 backdrop-blur-sm ${draggedTask === task.id ? "opacity-50 scale-95 rotate-2" : ""
                              }`}
                          >
                            <CardContent className="p-3">
                              <div className="flex items-start justify-between mb-2">
                                <h4 className="font-medium font-mono text-sm text-slate-200 flex-1">{task.title}</h4>
                                <div className={`w-2 h-2 rounded-full ${getPriorityColor(task.priority)} ml-2 mt-1`} />
                              </div>

                              {task.description && (
                                <p className="text-xs text-slate-400 font-mono mb-3 line-clamp-2">{task.description}</p>
                              )}
                              ...
                              <div className="text-xs text-slate-500 font-mono mt-2">{task.dueDate}</div>
                            </CardContent>
                          </Card>
                        )
                      })}
